    ...
    var body: some Scene {
        ...
        .commands {
            ...
            CommandGroup(after: .toolbar) {
                Button("Сканировать") { model.scan() }
                    .keyboardShortcut("r", modifiers: .command)
                Button("Удалить выбранное") { model.requestClean() }
                    .keyboardShortcut(.delete, modifiers: .command)
                Divider()
                Button("Только безопасные") { model.selectSafeOnly() }
                    .keyboardShortcut("1", modifiers: [.command, .shift])
                Button("Умная очистка") { model.selectSmartRecommended() }
                    .keyboardShortcut("2", modifiers: [.command, .shift])
                Button("Снять всё") { model.deselectAll() }
                    .keyboardShortcut("0", modifiers: [.command, .shift])
                Divider()
                Button("Свернуть детали") { model.collapseExpanded() }
